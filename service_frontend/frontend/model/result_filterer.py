...
from frontend.model.data_types import Result, ResultIterator
from frontend.model.filters import Filter
...
class ResultFilterer:
    ...
    def filter(self, results: ResultIterator) -> List[Result]:
        ...
        while len(filtered) < 100:
            try:
                result = next(results)
                if all([f.filter(result) for f in self._filters]) and not result.hidden:
