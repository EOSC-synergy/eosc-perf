...
from frontend.controller.json_result_validator import JSONResultValidator
...
class JSONResultValidatorTest(unittest.TestCase):
    ...
    def test_cray_sample(self):
        print(Path.cwd())
        try:
            with open("frontend/tests/controller/c-ray-result.json") as file:
                sample = file.read()
        except (OSError, JSONDecodeError) as e:
            self.fail("failed to open or parse sample result")
        try:
            with open("frontend/tests/controller/c-ray-template.json") as file:
                template = file.read()
        except (OSError, JSONDecodeError) as e:
